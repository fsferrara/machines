...
The current UTM version starts `qemu` with the `security_model=mapped-xattr` which uses extended attributes to store guest file permissions. This is a great solution if you modify files only from the guest vm.
Because of this limitation, I'll go with SPICE WebDAV that is a suboptimal solution.
